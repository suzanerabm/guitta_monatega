...
import {
	CharacterInfoPanel,
	type CharacterInfo,
} from "@/components/bichittos/CharacterInfoPanel";
...
export interface CharacterStripCharacter {
	...
	image: string;
	/**
	 * Ficha já resolvida pelo servidor. Quando presente, o card fica clicável e
	 * abre o CharacterInfoPanel. Ausente = card só decorativo.
	 *
	 * Vem por item para o strip não precisar importar os dados de
	 * personagem — era assim que a lore inteira, inclusive a de
	 * `visible: false`, entrava no bundle.
	 */
	info?: CharacterInfo;
}
...
export function CharacterStrip({
	...
	const allCards = effectiveNoLoop
		? characters
		: [...characters, ...characters];

	// Nomes que têm ficha resolvida. Só esses cards são interativos.
	const activatableNames = useMemo(
		() => new Set(characters.filter((c) => c.info).map((c) => c.name)),
		[characters],
	);

	// A chave do card é `${name}-${index}` sobre `allCards`, então o índice
	// resolve o item exato — inclusive na segunda cópia da lista duplicada do
	// loop. É mais preciso que a busca por nome que existia antes, que não
	// distinguia dois personagens de mesmo nome.
	const activeInfo = useMemo(() => {
		if (!activeKey) return null;
		const idx = Number(activeKey.slice(activeKey.lastIndexOf("-") + 1));
		return allCards[idx]?.info ?? null;
	}, [activeKey, allCards]);
		...
		>
			{sectionTitle && (
				<Heading
					...
					fontSize={
						mobileColor
							? { base: "section", md: "0.85rem", lg: "section" }
							: "section"
					}
					letterSpacing="wider"
